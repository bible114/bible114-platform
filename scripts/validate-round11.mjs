...
for (const text of ['5초만에 빠른 시작', '카카오로 시작', '구글로 시작', '기존 진도·달란트를 연결', '로그인 없이 둘러보기', '공동체 등록하기']) assert.match(login, new RegExp(text.replace(/[()·]/g, '\\$&')));
assert.doesNotMatch(login, /카카오톡으로 로그인|Google로 로그인|기존 회원 로그인 \(이름·생년월일로\)/);
assert.match(login, /공동체 관리자/);
assert.match(login, /카카오 또는 구글로 로그인/);
