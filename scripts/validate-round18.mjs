...
assert.match(actions, /\{ \.\.\.previous, \.\.\.response\.state\.user \}/);
...
assert.match(actions, /response\.state\.rosters\.map\(item => \[item\.orgId, item\.talent\]\)/);
assert.doesNotMatch(actions, /refreshedExtraOrgs = \[\]/);
assert.match(quiz, /const response = await submitQuiz\(/);
assert.match(quiz, /response\.state\.rosterTalents\.map\(\(\{ orgId, talent \}\) => \[orgId, talent\]\)/);
assert.match(quiz, /\[response\.state\.progressKey\]: freshProgress/);
