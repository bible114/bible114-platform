...
const fixture = (overrides: Record<string, unknown> = {}) => ({
  ...
  talentShop: {
    ...
    markets: {
      "kids-market": {
        enabled: true,
        items: [{ id: "snack", name: "간식", price: 7 }],
      },
    },
  },
  ...overrides,
...
const rejects = (code: string, overrides: Record<string, unknown>) => {
  try {
    validatePurchase(fixture(overrides));
    throw new Error("expected rejection");
  } catch (error) {
    assert(
      error instanceof PurchaseValidationError && error.code === code,
      `expected ${code}`,
    );
  }
};
...
Deno.test("서버 상품 가격과 user 지갑으로 구매를 계산한다", () => {
  ...
  assert(
    result.item.price === 7 && result.nextTalent === 3 &&
      result.walletKind === "user",
    "canonical purchase expected",
  );
});
Deno.test("개인계정은 roster 지갑을 사용한다", () => {
  ...
  assert(
    result.walletKind === "roster" && result.nextTalent === 1,
    "roster debit expected",
  );
});
Deno.test("위조 상품, 부서, 부족 잔액, 삭제 사용자를 거부한다", () => {
  ...
  rejects("INSUFFICIENT_TALENT", {
    user: { role: "member", churchId: "c1", departmentId: "kids", talent: 1 },
  });
  rejects("USER_UNAVAILABLE", {
    user: {
      role: "member",
      churchId: "c1",
      departmentId: "kids",
...
Deno.test("레거시 shared 시장도 서버 상품을 사용한다", () => {
  const result = validatePurchase(fixture({
    ...
    talentShop: {
      enabled: true,
      items: [{ id: "snack", name: "간식", price: 4 }],
    },
  }));
  assert(
    result.item.price === 4 && result.nextTalent === 6,
    "legacy purchase expected",
  );
});
