...
import { buildFirebaseClaims, buildGoogleAccessClaims, exchangeKakaoProfile, kakaoLinkDocumentId } from './core.ts';
...
const getServiceAccount = () => {
  ...
  return serviceAccount as { client_email: string; private_key: string; project_id: string };
};
...
const createGoogleAccessToken = async () => {
  ...
  if (!response.ok || !payload.access_token) throw new Error('GOOGLE_ACCESS_TOKEN_FAILED');
...
const verifyFirebaseIdToken = async (idToken: string) => {
  ...
  if (!payload.sub || typeof payload.sub !== 'string') throw new Error('FIREBASE_ID_TOKEN_INVALID');
  const signInProvider = (payload as { firebase?: { sign_in_provider?: string } }).firebase?.sign_in_provider;
  if (signInProvider === 'anonymous') throw new Error('FIREBASE_ID_TOKEN_ANONYMOUS');
  return payload.sub;
};
...
const getKakaoLink = async (kakaoId: string, accessToken: string) => {
  ...
  const response = await fetch(`https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/kakaoLinks/${docId}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  if (response.status === 404) return null;
  const payload = await response.json();
  if (!response.ok) throw new Error('KAKAO_LINK_READ_FAILED');
  return payload.fields?.uid?.stringValue ? String(payload.fields.uid.stringValue) : null;
};

const saveKakaoLink = async (kakaoId: string, uid: string, accessToken: string) => {
  const { project_id: projectId } = getServiceAccount();
  const docId = encodeURIComponent(kakaoLinkDocumentId(kakaoId));
  const response = await fetch(`https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/kakaoLinks/${docId}?currentDocument.exists=false`, {
    method: 'PATCH',
  ...
  try {
    ...
    if (linkIdToken) {
      ...
      if (linkedUid && linkedUid !== verifiedUid) {
        return json(origin, 409, { error: '이미 다른 계정에 연결된 카카오 계정입니다.' });
      }
      if (!linkedUid) {
        const created = await saveKakaoLink(profile.id, verifiedUid, accessToken);
        if (!created) {
          const racedUid = await getKakaoLink(profile.id, accessToken);
          if (racedUid !== verifiedUid) return json(origin, 409, { error: '이미 다른 계정에 연결된 카카오 계정입니다.' });
        }
      }
      return json(origin, 200, { linked: true });
    }

    const token = await createFirebaseCustomToken(linkedUid || `kakao:${profile.id}`);
