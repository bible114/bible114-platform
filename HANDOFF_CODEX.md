...
> **현재 활성 작업: "🔁 라운드 11" 체크리스트 (T60~T64, 카카오 로그인 + 첫 화면 단순화).** 라운드 1~10은 전부 완료·리뷰 통과됨.
...
## 🔁 라운드 11 — 카카오 로그인 + 첫 화면 단순화 (2026-07-12 사용자 확정)

> 배경: 사용자가 첫 화면 복잡도를 3회 지적. 업계 조사(토스·당근·배민·갓피플성경) 결론 —
> "로그인 vs 가입"을 사용자에게 묻지 않고, 한국 서비스의 정답은 **"카카오로 시작하기" 버튼 하나**.
> 카카오 = 신원만 확인(로그인+가입 통합), 소속은 라운드 9의 공동체 온보딩이 받는다.
> 아키텍처 적합: 카카오 신규 = accountType 'personal' — 라운드 9 개인 계정 경로를 그대로 재사용한다.

### 라운드 11 제약

- **firestore.rules 수정 금지** — 불필요 확인됨 (OIDC 로그인은 `sign_in_provider != 'anonymous'`라 isRealUser 통과, users create의 role 화이트리스트도 그대로).
- **카카오 JS SDK 금지** — Firebase compat `firebase.auth.OAuthProvider('oidc.kakao')`만 사용 (구글 로그인과 같은 패턴, 별도 SDK 불필요).
- 기존 로그인·가입 경로는 **제거하지 말고 위치만 강등** — 기존 교인·무소속·개인 계정 전부 지금 방식으로 계속 로그인 가능해야 한다.
- 실 카카오 팝업 테스트는 M10(사용자 수동) 완료 후에만 가능 — 그 전에는 provider id `oidc.kakao` 기준으로 코드만 작성하고 미검증 명시.

### 라운드 11 체크리스트

- [ ] **T60. 카카오 로그인 코어** (`useAuth.js`)
  - `handleKakaoStart`: `new firebase.auth.OAuthProvider('oidc.kakao')` + `signInWithPopup`. **카카오톡 인앱 브라우저에서는 popup이 아니라 `signInWithRedirect`** + 앱 로드 시 `getRedirectResult` 처리 (기존 `isKakaoTalkBrowser()` 재사용 — 카카오 로그인은 카톡 인앱에서 막히지 않으므로 구글의 차단 안내 문구를 재사용하지 말 것).
  - 성공 후 흐름은 `handleGooglePersonalSignup`(T50)과 동일 골격 재사용: users 문서 있으면 `openExistingPersonalUser`, 없으면 `buildPersonalUser` 기반 신규 생성(transaction race 검사 포함). in-flight ref·auth flow guard·uid 재검사 패턴 동일 적용.
  - 신규 생성 시 이름: OIDC nickname claim(`cred.user.displayName`) 사용, 없거나 공백이면 **이름 입력 1회 화면**을 끼운다 (랭킹·명부에 표시될 실명이므로 "실명(성함)을 입력해주세요" 안내). email은 카카오가 주면 저장, 없으면 null.
  - 기존 교회 교인이 카카오로 들어와 "새 계정"이 생기는 혼동 방지: 신규 생성 직전 확인 문구 1줄 — "처음 오셨죠? 새 계정을 만들어요. 기존 회원은 '기존 회원 로그인'을 이용하세요" + 취소 가능.
- [ ] **T61. 첫 화면 단순화** (`LoginView.jsx`)
  - 로그인 카드 기본 화면을 3항목으로 축소:
    1. **[💬 카카오로 시작하기]** — 카카오 공식 노란색(#FEE500, 텍스트 #191919), 카드에서 가장 크고 유일한 강조 버튼. 로그인+가입 통합임을 아래 한 줄로: "처음이어도, 다시 오셨어도 이 버튼 하나예요".
    2. "기존 회원 로그인 (이름·생년월일로)" — 텍스트 링크급. 클릭 시 현재의 로그인 체계(교회 검색 / 개인 계정, 기존 entry·뱃지·폴백 로직 전부 이 안으로 이동·보존)로 진입. 그 화면 안에 "처음인데 카카오가 없어요 → 가입" 링크로 기존 가입 경로도 유지.
    3. "로그인 없이 둘러보기" — 게스트(기존 로직).
  - 성도/교회관리자 탭 제거 → 하단 미세 링크 "교회 관리자 로그인 · 비밀번호 문의"로 강등 (admin 탭 로직은 보존, 진입점만 변경).
  - 재방문 기억(saveLastChurch)이 있는 사용자는 지금처럼 해당 로그인 폼으로 바로 — 단 그 폼에도 "카카오로 시작하기"를 작게 병기하지 말 것(선택지 최소화, 첫 화면으로 돌아가는 "다른 방법" 링크만).
  - `?church=ID` 전용 링크 진입 시에는 교회 로그인 폼을 우선 유지 (교회 배포 QR·안내문과의 호환).
- [ ] **T62. 카카오 신규 → 온보딩 연결**
  - 신규 카카오 계정이 성경 버전 선택 → 공동체 온보딩(T51)으로 자연스럽게 이어지는지 확인·연결. 게스트 진도 이관(T10 `shouldMigrateGuestState`)도 카카오 신규 생성에 동일 적용.
- [ ] **T63. 관리자·지원 표시**
  - 플랫폼 관리자 회원 목록: 카카오 personal 계정에 provider 표시(T53 뱃지 확장 — "카카오"), 비밀번호 없음(재설정 불가·카카오로 로그인 안내) 표시. Google personal과 동일 취급.
- [ ] **T64. 검증**
  - 빌드 + 첫 화면 3항목/기존 경로 보존/관리자 링크 강등을 로컬 브라우저로 확인, redirect 결과 처리·이름 입력 분기 픽스처. 실 카카오 팝업·계정 생성은 M10 전제라 미검증 명시. 사용자 실검증 시나리오를 메모란에 정리.
...
7. **Firebase 콘솔** → Authentication → Sign-in method → 새 공급자 → **OpenID Connect**: 이름 `kakao`(공급자 ID가 `oidc.kakao`가 되는지 확인), 발급자(issuer) `https://kauth.kakao.com`, 클라이언트 ID = 카카오 **REST API 키**, 클라이언트 보안 비밀번호 = 5번의 Client Secret.

**라운드 11 보류(착수 금지)**: 기존 교인 계정에 카카오 연결(계정 통합 — 라운드 12 후보), 카카오싱크(간편가입 약관 동의), 휴대폰 번호 인증.
