...
| 2026-07-17 | T128b 읽기 일정 66권 전장 커버리지 검사 | `scripts/validate-schedule-coverage.mjs`, `package.json`, `HANDOFF_CODEX.md` | 클로드가 운영 본문 4건과 일정·퀴즈 인덱스를 먼저 복구한 상태를 이어받아, 앱의 `parseReadingRange`와 같은 계약으로 whole_bible 66권 1,189장·new_testament 27권 260장의 파싱 불가 및 누락을 실패 처리하는 검사를 추가했다. 기대 장 수는 일정에서 역산하지 않고 고정 정경 메타데이터로 검증하며 `npm run validate`에 편입했다. 전체 validate(platform-api 436 tests), build, diff 검사 통과. Edge·웹 배포와 push는 하지 않았고 캐시 title 9건은 메모의 순서 게이트대로 Edge 배포 뒤로 남겼다. |
...
- **⚠️ 남은 2단계 (Edge 배포 후에만)**: 캐시 title 9건 교정(파서 인식용). 대상·기대 현재값·새값은 `~/Developer/클로드/bible114-verses-title-phase2-20260717.json`에 저장됨(적용 전 expectedCurrentTitle 일치 확인 필수). **순서 강제**: 새 quiz-answer-index를 포함한 Edge 배포가 먼저 — 배포 전에 title을 바꾸면 클라이언트가 새 문항을 내밀고 구 인덱스가 invalidQuiz로 거부한다. title 반영 전까지 퀴즈 고아 24문항은 실사용에서 계속 미출제(회귀는 아님).
