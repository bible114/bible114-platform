...
> **현재 활성 작업: T125e-2d·e-3 → 라운드 27 T128a/b → T127 최종 직접 writer 감사**. T123 읽기·퀴즈, T125d `publicChurches`, T125e-1 검색 노출, T125e-2a/b 신규 관리자 교회 생성·입장코드·무소속 점검, T125e-2c 공동체 이름 변경, T127a~h(재시작·업적·긴급 rules 축소·개인 지갑·레거시 진도 정규화·최초 소속 설정·혼자 읽기 참여·개인 계정 전환)와 2026-07-16 Kakao 관리자 가입 묶음은 완료했다. 단, T125e-2c는 아직 로컬 커밋 상태로 운영 미배포다. T126 영상 경로의 기존 관찰 시계(2026-07-23 05:14 KST)는 유지하고, 읽기·퀴즈·T125·T127a~h를 포함한 **최종 T127 7일 시계는 전체 릴리스 완료 시각인 2026-07-16 19:26 KST부터 다시 시작해 2026-07-23 19:26 KST 전에는 닫지 않는다**. 그 전에는 일반 공동체 계정의 보호 필드 호환 상한, roster 직접 진도 writer, legacy `videoAutoConfig.apiKey`를 닫거나 정리하지 않는다.
...
- **⚠️ 남은 2단계 (Edge 배포 후에만)**: 캐시 title 10건 교정(파서 인식용, T129 sequential Day 357 포함). 대상·기대 현재값·새값은 `~/Developer/클로드/bible114-verses-title-phase2-20260717.json`에 저장됨(적용 전 expectedCurrentTitle 일치 확인 필수). **순서 강제**: 새 quiz-answer-index를 포함한 Edge 배포가 먼저 — 배포 전에 title을 바꾸면 클라이언트가 새 문항을 내밀고 구 인덱스가 invalidQuiz로 거부한다. title 반영 전까지 퀴즈 고아 문항은 실사용에서 계속 미출제(회귀는 아님).
...
## [ ] T129. `1year_sequential` 퀴즈 일정 분리 (2026-07-17 운영 영향 감사)
...
- [ ] **T129d. 배포·실계정 스모크** — 새 인덱스를 포함한 Edge를 먼저, 웹을 다음에 배포한다. sequential 실계정 1개에서 현재/방금 완료 Day 퀴즈 제출이 `invalidQuiz` 없이 처리되고 원장이 1건 생기는지 확인한다. 배포·운영 계정 사용은 사용자 명시 승인 뒤에만 한다.
