...
| 2026-07-23 | T143 공동체 관리자 화면 투어 전면 개편 | `src/components/{ChurchAdminTutorial,ChurchAdminView}.jsx`, `src/components/churchAdmin/{DashboardTab,TalentShopTab}.jsx`, `scripts/validate-admin-tour.mjs`, `package.json` | 실제 6개 탭을 4개라고 설명하고 사라진 정렬·행별 관리 버튼을 가리키던 기존 관리자 투어를 제거했다. 공동체 관리 상단→6개 메뉴→목양 대시보드→교인 관리→달란트 상점→조직→공지→가입 안내·입장코드 순서의 8단계로 다시 만들고, 상단 버튼을 `관리자 화면 투어`로 명확히 표시했다. 탭을 단계에 맞춰 자동 전환하고 대상이 없을 때도 중단되지 않으며, 모바일 하단 안내판·데스크톱 화면 경계 내 자동 배치를 적용했다. 전용 검사·build·diff-check 통과, 저장되지 않는 로컬 모의 관리자 화면에서 8개 단계의 자동 탭 전환·안내판 경계·콘솔 오류 0을 확인한 뒤 모의 파일은 삭제했다. |
| 2026-07-23 | T142 앱 화면 투어 전면 개편 | `src/components/{DemoTour,TutorialOverlay,LoginView}.jsx`, `src/components/dashboard/{DashboardHeader,DailyVideoCard}.jsx`, `scripts/validate-app-tour.mjs`, `package.json` | 실제 화면에 없는 점수·레벨 단계와 닫힌 메뉴 내부를 직접 가리키던 단계를 제거하고, 통독 계획→메뉴→영상→읽기 범위→낭독→본문→선택 퀴즈→읽기 완료→묵상 순서의 9단계로 다시 만들었다. 로그인 첫 화면에 `앱 화면 먼저 둘러보기`를 복구하고, 예시 화면을 현재 대시보드와 같은 구성으로 단순화했다. 대상이 없거나 숨겨진 경우 이전 강조 영역이 남지 않으며, 모바일은 하단 안내판·데스크톱은 화면 안쪽 자동 배치로 처리했다. 전용 검사·build 통과, 로컬 브라우저에서 9개 단계 대상·이전/다음·모든 안내판 화면 경계 내 표시·콘솔 오류 0을 확인했다. |
| 2026-07-23 | T141 FAQ 정리·소셜 미연결 계정 감사 | `src/components/modals/ReadingGuideModal.jsx`, `scripts/{validate-round29,audit-unlinked-social-accounts}.mjs` | FAQ에서 현재 로그인 구조와 맞지 않는 `로그인이 안 돼요`, `비밀번호를 잊었어요` 2개를 삭제했다. 운영 Firestore·Firebase Auth·`kakaoLinks`를 읽기 전용으로 대조하는 감사 도구를 추가했고, 활성 users 57명 중 구글 연결 1명·카카오 연결 3명·어느 쪽도 연결되지 않은 users 53명(기존 Auth 있음 47명, Auth 없음 6명)을 확인했다. 결과에는 UID·이메일·생년월일·비밀번호를 포함하지 않았으며 운영 데이터 쓰기는 없었다. |
| 2026-07-23 | T140 날짜 설정 시편 단위 표기 | `src/components/modals/DateSettingsModal.jsx`, `src/utils/readingRangeDisplay.js`, `scripts/validate-reading-range-display.mjs`, `package.json` | 날짜 설정 달력의 읽기 범위에만 표시 변환을 적용해 `시 1장`→`시편 1편`, `시 1-7장`→`시편 1-7편`, 복합 범위의 `시 90장`→`시편 90편`으로 보이게 했다. 다른 책의 `장` 표기, 일정 원본, 본문·퀴즈 파싱은 변경하지 않았다. 전용 6개 사례 검사·전체 validate·build·diff-check 통과. |
...
| 2026-07-18 | T124 관리자 action 재처리 P1 보강 + T124d 전용 실스모크 도구 준비 | `supabase/functions/platform-api/{adminPurchaseCore,index}*`, `scripts/{run-t124d-smoke,validate-t124d-smoke,validate-department-talent,validate-round24}.mjs`, `package.json` | 판매·수령·환불의 같은 requestId 재처리가 ledger 입력/result만 맞으면 실제 구매가 누락·역전·손상돼도 완료로 보이던 ghost replay를 수정했다. canonical 구매 상태·`adminActionRequestId`·actor/target·v2 지갑 snapshot·환불 금액/전후 잔액·최신 지갑을 대조하고 모순은 409로 닫으며, 정상 응답 유실 replay는 최신 잔액을 반환한다. 관리자 1명·회원 3명·초기 3달란트의 숨김 일회용 fixture로 일반 구매 2건→창구판매/수령/환불과 같은 UUID replay→구매/ledger/잔액 독립 감사→users/church/Auth 조건부 완전 정리를 수행하는 도구를 추가했다. 공개 목록·meta·platformStats 쓰기 0, 모든 삭제 updateTime fence, exact confirm·0600 manifest. 생성 실패 rollback은 256-bit 소유 토큰과 예상 11개 문서 exact 집합이 모두 맞을 때만 원자 삭제하며 collision은 Firestore 삭제 0이다. 운영 읽기 전용 create preflight, 전체 validate 446/446, build, diff 검사 통과. 운영 생성·Edge 배포·웹 배포·push 없음이며 T124d 체크는 실제 배포·실로그인·스크린샷 전까지 유지한다. |
...
| 2026-07-17 | T129 순서대로 읽기 퀴즈 전용 일정·서버 scope 로컬 복구 | `scripts/audit-sequential-{quiz-impact,verses-schedule}.mjs`, `src/data/{sequential_schedule.json,schedules.js}`, `scripts/{generate-quiz-answer-index,validate-schedule-coverage,validate-quiz,validate-round18,validate-round24}.mjs`, `supabase/functions/platform-api/{quizCore,quiz-answer-index}*`, `HANDOFF_CODEX.md` | 읽기 전용 운영 집계에서 active users 37명 중 `1year_sequential` 5명(member 4, churchAdmin 1), 이들의 퀴즈 진행·성공 원장 0건을 확인했다. 운영 sequential 캐시 365/365의 본문 장 헤더는 66권 1,189장 전체이며 title과 364일 일치했고 Day 357 제목만 장 번호가 없어 파싱 실패했다. 본문 헤더로 전용 365일 일정을 생성하고 클라이언트 일정·퀴즈 인덱스 `allowed.sequential`·서버 exact scope를 분리했다. Day 357 제목 교정도 Edge 이후 title phase2 목록에 추가했다(총 10건). 전체 `npm run validate`(platform-api 442 tests), build, diff 검사 통과. Edge→웹→title 순서 배포와 실계정 스모크는 사용자 승인 대기. 클라이언트 `platformStats` 직접 쓰기는 0건이나 회원 편집·탈퇴·공동체 관리자 운영 writer는 T127 7일 관찰 호환 창 때문에 남아 있고, 삭제된 platformAdmin 권한 결속도 별도 rules 세션 대상이다. |
| 2026-07-17 | T125e-3 플랫폼 통계 서버 원장 완결 | `supabase/functions/platform-api/{index,memberSignupCore,completeChurchAdminSignupService,platformStatsService}*`, `src/{hooks/useAuth.js,components/LoginView.jsx}`, `scripts/validate-{signup-consent,church-lifecycle}.mjs`, `HANDOFF_CODEX.md` | 일반 교인·개인·공동체 관리자 가입이 동일 transaction에서 `total_readers`를, 공동체 생성은 `total_churches`도 한 번만 증가시킨다. 기존 브라우저 통계 increment 3곳을 제거했고, 무소속 가상 공동체 가입도 직접 users 생성 대신 서버 `completeMemberSignup`으로 통합해 누락 경로를 없앴다. 통계가 없으면 원장을 만들고 기존 카운터가 손상됐으면 추정하지 않고 관리자 재계산을 요구한다. 관리자 dry-run/확인/실행 재계산, 공동체 비활성화 증감, KST 당일 독자·누적 완독 의미와 랜딩 라벨까지 확정했다. 신규 통계/무소속/공동체 가입 단위 검증 25건, 가입·lifecycle 계약 검사, Deno check 통과. 배포·push 없음. |
| 2026-07-17 | T125e-2d 복원 가능한 공동체 비활성화 + T125e-3 재계산 기반 | `supabase/functions/platform-api/{adminChurchLifecycleService,platformStatsService,core,index}*`, `src/{components/PlatformAdminView.jsx,components/dashboard/CommunityMembershipCard.jsx,hooks/useUserAuth.js,utils/platformApi.js}`, `scripts/validate-church-lifecycle.mjs`, `HANDOFF_CODEX.md` | 공동체를 검색·가입에서 먼저 차단한 뒤 주 소속 일반 사용자만 generation 표식으로 400건씩 soft-delete하고, 개인/외부 roster·양수 달란트·pending 구매는 동결 보존한다. 같은 generation만 복원하며 공동체는 사용자 복원 뒤 마지막에 다시 연다. 비활성화/복원 시 total_readers·total_churches를 불변 원장과 함께 반영하고, 관리자에게 잔액·pending 정산 수치를 읽기 전용으로 표시한다. `rebuildPlatformStats({dryRun})`의 KST 당일 독자·현재 활성 교회/독자·누적 완독 전수 재계산과 snapshot fence도 추가하고 기존 관리자 직접 덮어쓰기 버튼을 교체했다. 플랫폼 API 438 tests, lifecycle validator, build, diff 검사 통과. 가입 경로의 client `total_readers` increment 3곳은 서버 가입 action 이관 전까지 T125e-3 잔여로 남긴다. 배포·push 없음. |
| 2026-07-17 | 카카오 콜론 UID 운영 쓰기 프로브 | `scripts/verify-kakao-uid-write-path.mjs` (실행만), `HANDOFF_CODEX.md` | 임시 `kakao:` Auth/users 픽스처로 배포 v10의 진도 보정과 activityActions 원문 UID 쓰기를 실검증해 `VERIFY RESULT: PASS`. 잘못된 `%` 문서 HTTP 404, 원장 1건·users 문서·Auth 계정 정리 3단계 모두 성공(잔여 없음). 실제 사용자 카카오 로그인 대시보드 진입 확인만 사용자 확인 대기. |
| 2026-07-17 | T128b 읽기 일정 66권 전장 커버리지 검사 | `scripts/validate-schedule-coverage.mjs`, `package.json`, `HANDOFF_CODEX.md` | 클로드가 운영 본문 4건과 일정·퀴즈 인덱스를 먼저 복구한 상태를 이어받아, 앱의 `parseReadingRange`와 같은 계약으로 whole_bible 66권 1,189장·new_testament 27권 260장의 파싱 불가 및 누락을 실패 처리하는 검사를 추가했다. 기대 장 수는 일정에서 역산하지 않고 고정 정경 메타데이터로 검증하며 `npm run validate`에 편입했다. 전체 validate(platform-api 436 tests), build, diff 검사 통과. Edge·웹 배포와 push는 하지 않았고 당시 캐시 title 9건은 메모의 순서 게이트대로 Edge 배포 뒤로 남겼다(T129에서 sequential 1건 추가되어 현재 10건). |
| 2026-07-17 | T125e-2c 공동체 이름 변경·로그인 점진 보정 | `supabase/functions/platform-api/{core,index,adminChurchRenameService,normalizeLegacyReadingPositionService}*`, `src/{components/PlatformAdminView.jsx,hooks/useUserAuth.js,utils/platformApi.js}`, `scripts/validate-{church-lifecycle,public-directory,round24}.mjs`, `HANDOFF_CODEX.md` | platformAdmin/superAdmin 전용 `adminRenameChurch`가 공동체 원본 이름·최소 legacy/public 투영·불변 관리자 원장을 한 transaction에 반영한다. strict 요청/응답, 삭제·가상 공동체 거부, no-op UUID 미소비, 최대 3회 409와 응답 유실 replay, 누락 public 및 legacy 비밀 drift 복구를 검증했다. 기존 users/roster 이름은 fanout하지 않고 모든 로그인에서 실행되는 서버 진도 감사가 기준 `churches.name`과 다른 `users.churchName`만 점진 보정하며, 클라이언트는 source-server 재조회 결과만 적용한다. 전체 `npm run validate`(platform-api 436 tests), build, Deno check/fmt, diff 검사를 통과했다. 실제 관리자 이름 변경·로그인 보정은 운영 데이터를 바꾸므로 미실행했고 Edge·웹 배포와 push는 하지 않았다. |
| 2026-07-17 | 카카오 관리자 로그인 분기 수정 운영 배포 | `src/hooks/useAuth.js`, `scripts/validate-kakao-custom-auth.mjs` (`cffa539`) | 사용자 지시로 `main` push 후 `npm run deploy`(gh-pages). 공개 번들 `assets/index-DohYQKq4.js` HTTP 200, 로컬 dist와 SHA-256 `3a8f6bd7…e7ca4` 일치. 클라이언트만 변경이라 Edge·rules 재배포 없음, T126/T127 관찰 시계 영향 없음. 실제 Kakao OAuth 재시도는 사용자 확인 대기. 부수 발견: apex `bible114.net`은 A/AAAA 레코드가 없어 미해석(NXDOMAIN에 준함) — 서비스는 `www.bible114.net`만 유효하며 Safari 주소창이 www를 숨겨 apex처럼 보인다. 필요시 Google Cloud DNS에 GitHub Pages A 4개(185.199.108~111.153) 추가 검토. |
...
- 다음 T124d는 기존 `run` 한 번으로 끝내면 서버 action은 검증되지만 실제 `ChurchAdminView` 버튼→`platformApi` 경로의 Network 증거가 남지 않는다. 따라서 runner를 `create → prepare`(회원 2명의 일반 pending 구매만 생성)와 `audit-ui`(동적으로 생성된 관리자 requestId/ledger 3건을 검색·결속)로 분리한다. 준비 단계는 관리자 action을 절대 호출하지 않는다.
- 공개 UI에서는 private manifest의 일회용 관리자로 로그인해 ① 제3회원 1달란트 창구 판매 ② 첫 pending 수령 완료 ③ 둘째 pending 취소·환불을 각각 확인창까지 눌러 실행한다. 각 버튼에서 `platform-api` action·HTTP 200을 Network로 확인하고 판매/수령/환불 전후 화면을 캡처한다. 독립 audit은 구매 정확히 3건(delivered 2, cancelled 1), admin ledger 정확히 3건, actor/target/purchase/requestId, 잔액 3/2/3/2를 확인한다.
- 감사 통과 뒤 users→church subtree→Auth 순서 cleanup과 deep residue 0, 공개 목록/meta/stats 정리 직전·직후 불변을 확인한다. 그때만 T124d를 `[x]`로 닫는다. 이후 지금 날짜에 가능한 Codex 독립 작업은 없고, T126 조건부 실환경 2건은 실제 수동 영상/운영 platformAdmin 세션이 생길 때 확인한다. T132는 2026-07-24 19:00:53 KST 이후 별도 명시 승인 전까지 계속 금지다.

2026-07-18 T124 관리자 action ghost replay P1 로컬 수정 + T124d 안전 실행 준비:
- T124d 실환경 스모크 설계 감사 중 판매·수령·환불의 기존 재처리가 `talentAdminActions/{requestId}`의 일부 입력과 저장 result만 확인하고 실제 구매의 현재 상태를 결속하지 않는 P1을 찾았다. 서비스 계정 drift나 손상으로 구매가 누락·역전돼도 ghost 완료를 반환할 수 있어 운영 스모크보다 먼저 수정했다.
- 새 순수 replay 검증은 canonical 구매 존재/상태, `adminActionRequestId`, actor/target, 판매 snapshot, v2 `walletKind/walletOrgId`, 환불 금액과 balanceBefore/After, 최신 canonical 지갑과 안전 잔액을 모두 대조한다. 정상 개인 전환 `user → 같은 공동체 roster`는 명시적 2차 확인 원장일 때만 허용하고, 나머지 불일치는 `PURCHASE_REPLAY_CONFLICT` 409다. 전체 플랫폼 API 테스트는 446/446이다.
- `scripts/run-t124d-smoke.mjs`는 고정 prefix의 숨김 일회용 교회만 허용하고 관리자 1명·회원 3명·초기 talent 3을 만든다. 공개 목록·`churchDirectory`·public meta·platformStats를 쓰지 않으며, 일반 구매 2건과 관리자 창구판매/수령/환불 3건, 동일 UUID replay, 구매 3·ledger 3·잔액 독립 감사를 수행한다. cleanup은 updateTime fence로 users→church subtree→Auth 순서이며 deep residue 0과 정리 직전/직후 보호 문서 불변을 확인한다. 생성 실패 rollback도 예상 11개 경로·256-bit 소유 토큰·updateTime이 모두 맞을 때만 원자 실행하고, collision/비소유 상태는 Firestore 삭제 0이다. exact 확인문구와 0600 private manifest가 없으면 쓰지 않는다.
- 읽기 전용 production create preflight와 새 안전 validator, 전체 validate 446/446, build, diff 검사를 통과했다. **아직 운영 fixture 생성·Edge/웹 배포·push는 하지 않았다.** T124d 완료 표시는 최신 platform-api 배포 후 이 도구로 서버 action을 확인하고 공개 관리자 UI 실로그인·Network action·결과 화면 스크린샷을 남긴 다음 cleanup residue 0까지 확인할 때만 한다. T132 날짜 게이트와 무관한 작업이며 T132는 시작하지 않았다.
...
- T127 시계 종료 전 기준선으로 `audit-t127-legacy-state.mjs`를 재실행했다. 2026-07-17 이전 집계와 동일하고 Day>365·percent-encoded UID·미이관 inventory/unlockedRooms·roster 경로/진도/고아 이상이 모두 0이다. 2026-07-24 19:00:53 KST 이후 동일 스크립트를 다시 실행해 drift 없음을 확인한 후 차단을 시작한다.
- T129d를 운영에서 종결했다. 일회용 Auth/users fixture(`1year_sequential`, Day 105)로 배포된 `submitQuiz`를 첫 시도 정답 제출해 HTTP 200 `ready`, attempts 1, solved true를 받았고 users `quizProgress`·`activityActions`·`quizAttemptSlots` 각 1건을 재조회했다. 하위 문서 2건·users 1건·임시 Auth는 실행 성공/실패와 관계없이 정리하는 경로로 모두 삭제됐다. 다음 코드 작업은 최종 T127 7일 시계가 끝나는 2026-07-24 19:00:53 KST 이후의 writer/rules 감사이며, 그 전에는 호환 writer를 닫지 않는다.
- `scripts/audit-sequential-quiz-impact.mjs`를 추가해 운영 Firestore를 읽기 전용으로 집계했다. active users 37명 중 `1year_sequential`은 5명(member 4, churchAdmin 1)이고, 다섯 계정 모두 `quizProgress`가 없으며 성공한 submit/skip 원장도 0건이다. 전체 성공 퀴즈 원장 5건은 `1year_easy` 2건·`nt_new` 3건이다. `invalidQuiz` 거부는 transaction 전에 끝나 원장이 생기지 않으므로 이 결과만으로 인과를 단정하지 않지만, 클로드가 확인한 순서대로 캐시↔whole 일정 전면 불일치와 함께 보면 T129 복구 우선순위가 높다.
- T125e-3 완료 후 `src`의 `settings/platformStats` 직접 쓰기는 0건이다. 다만 T127 호환 창 writer로 플랫폼 전체 회원 편집(`App.jsx`의 users 진도·소속 transaction), 개인 추가 공동체 탈퇴(`CommunityMembershipCard.jsx`의 roster delete/primary 변경), 공동체 관리자 회원 삭제·복원·진도/지갑/설정 writer, 일회성 private-access/directory 이전 도구가 남아 있다. 이들을 한꺼번에 rules로 닫으면 구버전 탭과 현재 관리 기능이 깨지므로 최신 웹 관찰 종료인 2026-07-24 19:00:53 KST 전에는 수정/차단하지 않는다.
- `firestore.rules`의 `isPlatformAdmin()`은 여전히 `isDeleted` 결속이 없다. 저장소 지침상 users read/rules는 별도 세션이므로 이번 작업에서는 건드리지 않았으며, T127 최종 rules 배포 묶음에서 반드시 처리해야 한다.

2026-07-17 T125e-2c 공동체 이름 변경 로컬 완료:
- `adminRenameChurch`는 비익명 platformAdmin/superAdmin을 transaction 안에서 다시 확인하고 `churches/{id}.name`, `settings/churchDirectory`, `publicChurches/{id}`, `platformAdminActions/{requestId}`를 한 commit에 쓴다. legacy 투영은 `{id,name,hidden?}`만 재구성해 남은 code/hash drift를 제거하고 public 누락은 precondition으로 복구한다. canonical no-op은 UUID를 소비하지 않으며 strict replay·입력 충돌·최대 3회 409·apply-then-409를 고정했다. 원본 이름은 이미 같지만 projection만 복구하는 경우도 동등 이름의 rename 원장으로 안전하게 replay한다.
- 플랫폼 관리자 상세 화면에 가상 공동체를 제외한 이름 변경 버튼을 추가했다. 브라우저는 교회·directory를 직접 쓰지 않고 strict API 응답 뒤 로컬 목록과 directory 캐시만 갱신한다. 기존 users/roster snapshot의 일괄 fanout은 하지 않는다.
- 기존 매 로그인 `normalizeLegacyReadingPosition` transaction이 일반 계정의 canonical `churches/{churchId}.name`을 함께 읽어 다른 `users.churchName`만 masked update한다. 보정이 있으면 클라이언트는 source-server users를 재조회해 최신 이름만 적용한다. 개인 계정과 churchId 없는 legacy는 건드리지 않으며, 기준 교회가 없거나 손상된 정상 진도 사용자는 기존 recoverable audit 경로로 로그인 가용성을 유지한다.
- 전체 `npm run validate`(platform-api 436 tests), `npm run build`, Deno check/fmt, 관련 validator와 `git diff --check`를 통과했다. 운영 데이터 변경이 필요한 실 관리자 rename/재로그인은 미실행이며 **Edge·웹 배포와 push는 하지 않았다.** 다음 첫 작업은 확정 정책에 따른 T125e-2d 복원 가능한 공동체 비활성화다.
...
### [x] T125. 입장코드·가입·공동체 참여·디렉토리·통계 이관
...
- [x] **T125e. 남은 운영 writer·통계 서버 권위** — 변경/숨김/삭제/신규 교회 writer를 서버 action으로 모은 뒤 `churchDirectory` 직접 write를 닫을 준비를 한다. `platformStats`는 필드 의미를 먼저 확정하고, 클라이언트 숫자 입력 없이 생성/읽기 ledger에서 한 번만 집계하며 platformAdmin `rebuildPlatformStats({dryRun})`로 전수 재계산·차이를 확인한다.
  ...
  - [x] **T125e-2. 신규/변경/삭제 교회 writer** — 신규 등록·입장코드·이름 변경·비활성화/복원을 서버 action으로 이관했다.
    ...
    - [x] **T125e-2c. 교회 이름 변경** — platformAdmin/superAdmin 서버 action이 `churches.name`·legacy/public 투영·불변 원장을 한 transaction에서 바꾸고, 기존 users/roster snapshot은 fanout하지 않는다. 로그인 진도 감사가 기준 교회 이름과 다른 `users.churchName`만 점진 보정하며 source-server 재조회 결과만 화면에 적용한다.
    - [x] **T125e-2d. 교회 삭제/비활성화** — platformAdmin/superAdmin 전용 `adminSetChurchLifecycle`가 원본·legacy/public 디렉토리를 먼저 닫고, 주 소속 일반 users만 세대 표식으로 400건씩 재개 가능한 soft-delete한다. 개인/외부 roster·달란트·pending 구매·Auth/private/원장은 보존하며 자동 환불하지 않고 정산 수치만 반환한다. 복원은 같은 세대 users만 되살린 뒤 공동체를 마지막에 열며 통계 증감과 불변 원장을 함께 확정한다. 개인 계정의 기준 공동체가 비활성 상태면 로그인은 유지하고 전환 안내를 표시한다.
  - [x] **T125e-3. `platformStats` 의미 확정·재계산** — 가입·공동체 생성·비활성화/복원 transaction에서 활성 독자·공동체를 한 번만 증감하고, KST 당일 독자와 누적 완독을 포함한 platformAdmin dry-run/execute 전수 재계산을 구현했다. 브라우저 직접 카운터 쓰기는 제거했고 랜딩 라벨은 `누적 완독`으로 정정했다.
...
2026-07-18 저녁 — 라운드 29 신설 (Claude 설계, 사용자 승인): 성도·관리자 안내 체계 개편 T133(가입 안내문 2면: 홈 화면 추가 + 매일 사용법)·T134(성도용 FAQ, 도움말 모달)·T135(관리자 매뉴얼 FAQ 8항목). 카피·레이아웃 시안은 `review/mockups/guide-faq-mockup-2026-07-18.html` (사용자 확인 완료). "매일 사용법은 별도 인쇄물 금지, 가입 안내문에 통합"이 사용자 확정 사항이니 새 인쇄 버튼을 만들지 말 것. T132 게이트와 무관하게 진행 가능.

2026-07-18 — T130 종결 (사용자 결정): 쉬운성경·새한글·메시지 버전은 **미운영 확정**, 클라이언트 코드 전면 제거 완료(작업 로그 참조). T130의 캐시 재생성 제안은 실행하지 않는다. 운영 `verses`의 `nt_saehangul_*`/`nt_easy_*`/`nt_message_*`/`1year_saehangul_*`/`1year_easy_*` 캐시 문서는 참조되지 않는 고아 데이터로 남으며, 삭제는 별도 사용자 승인 시에만 진행한다. 쉬운 **난이도** 퀴즈 파이프라인(quizNtEasy)은 유지 대상이니 혼동하지 말 것.
...
- **완료 (2026-07-17 운영 반영)**: 새 quiz-answer-index 포함 Edge v11과 웹을 먼저 배포한 뒤 캐시 title 10건을 expected current/updateTime CAS의 한 atomic title-only commit으로 교정했다. 적용 후 10/10 새 제목, parser 10/10 기대 일치, sequential title/body 365일·66권 1,189장 일치를 확인했다. 백업은 `/Users/jaeam/Developer/클로드/bible114-verses-title-phase2-backup-2026-07-17T10-04-34-107Z.json`(0600, SHA-256 `2d935788f35155bb05be0a52b293f5a39409fb6d6bee302209bd3c6036c06489`).
...
- [x] **T128a. 요이·요삼 표기 수정 (즉시, 무위험)** — `read_schedules.json` whole_bible Day 363의 range를 `"요일 3-5장, 요이 1장, 요삼 1장"`으로 변경. 독서 의미 불변이므로 verses 캐시 무관. `node scripts/generate-quiz-answer-index.mjs` 재생성 후 `2john-1-*`/`3john-1-*`의 `allowed.whole`에 363이 들어갔는지 확인. 전체 validate·build 통과로 완료.
- [x] **T128b. 66권 전장 커버리지 검사 신설** — `scripts/validate-schedule-coverage.mjs`가 앱과 같은 `parseReadingRange` 계약으로 whole_bible 66권 1,189장과 new_testament 27권 260장의 누락·파싱 불가 일정을 실패 처리한다. 정경 기대 장 수는 일정에서 역산하지 않고 고정 메타데이터로 검증하며 `npm run validate`에 포함했다. T128c 본문·일정 복구가 먼저 완료되어 allowlist는 두지 않았다.
- [x] **T128c. 범위 확장 (운영 본문 확인·복구 완료)** —
...
## [x] T129. `1year_sequential` 퀴즈 일정 분리 (2026-07-17 운영 영향 감사)

> 배경: `1year_sequential` 캐시는 창세기부터 계시록까지 정순이지만 앱·서버 퀴즈는 `whole_bible` 혼합 일정을 공유한다. 운영 읽기 전용 집계에서 활성 sequential 사용자 5명(member 4, churchAdmin 1), 퀴즈 진행·성공 원장 0건을 확인했다. 거부된 `invalidQuiz`는 원장을 만들지 않으므로 Edge 로그 없이는 실패 횟수를 집계할 수 없다.

- [x] **T129a. 운영 영향 분포 감사** — `scripts/audit-sequential-quiz-impact.mjs`로 개인정보를 출력하지 않고 active plan 분포, sequential 역할 분포, 퀴즈 진행 보유 수, 성공 원장을 집계했다.
- [x] **T129b. sequential 전용 365일 일정 원장 생성** — 운영 `verses/1year_sequential_{day}` 365/365의 title과 본문 장 헤더를 대조했다. 본문 기준 66권 1,189장 전체·누락 0이고 364일 title과 일치한다. Day 357만 제목에 장 번호가 없어 본문 헤더(요이·요삼·유다 각 1장)를 원장으로 사용하고 Edge 이후 title 교정 목록에 추가했다.
- [x] **T129c. 클라이언트·서버 퀴즈 scope 분리** — `SCHEDULE_DATA['1year_sequential']`을 전용 일정에 연결하고 quiz-answer-index에 `allowed.sequential`을 생성했다. 서버 `getQuizPlanScope()`는 exact `1year_sequential`만 sequential scope로 판정하며 revised/new/saehangul은 기존 whole을 유지한다. 기존 인덱스 strict 검증·퀴즈 단위 테스트·365일 최소 pool 검사를 확장했다.
- [x] **T129d. 배포·실계정 스모크** — 새 인덱스를 포함한 Edge v11 → 웹을 순서대로 배포했다. 기존 users/Auth 미존재 guard를 둔 일회용 운영 계정으로 sequential Day 105 서버 정답 첫 제출을 확인했고 HTTP 200 `ready`, attempts 1, solved true, `quizProgress`·`activityActions`·`quizAttemptSlots` 각 1건이었다. 검증 후 하위 문서·users·Auth를 모두 삭제했다(`VERIFY RESULT: PASS`).

완료 기준: sequential 365일 전장 커버리지·일정/본문 일치, 전체 `npm run validate`·build, Edge→웹 배포, sequential 실계정 퀴즈 원장 확인.

---

## 🔁 라운드 28 — 서비스 오픈 전 최종 마감 (2026-07-17 Claude 정리, 사용자 지시)

> 배경: T123·T125·T126·T127a~h·T128·T129 전부 운영 배포 완료. 최종 T127 7일 관찰 시계는 **2026-07-17 19:00:53 ~ 2026-07-24 19:00:53 KST**. 이 라운드는 (A) 시계와 무관하게 지금 진행 가능한 작업과 (B) 시계 종료 후에만 가능한 차단 시퀀스로 나뉜다. T130·T131은 지금 착수 가능, T132는 **2026-07-24 19:00:53 KST 이전 착수 금지 + 사용자 명시 승인 필수**.
>
...
## 🔁 라운드 29 — 성도·관리자 안내 체계 개편 (2026-07-18 Claude 설계·사용자 승인, Codex 구현)
...
### [x] T137. 카카오·구글 전용 로그인·기존 기록 연결 (2026-07-19 사용자 지시)

- 일반 로그인 첫 화면은 카카오·구글과 게스트만 노출하고, 이름·생년월일·비밀번호 및 관리자 이메일 로그인 입구는 제거한다.
...
### [x] T140. 날짜 설정 시편 `편` 표기 (2026-07-23 사용자 지시)

- 날짜 설정 달력의 읽기 범위 표시에서만 시편 약칭을 `시편`으로 풀고 단위를 `장`에서 `편`으로 바꾼다.
- 예: `시 1장`→`시편 1편`, `시 1-7장`→`시편 1-7편`, `신 32-34장, 시 90장`→`신 32-34장, 시편 90편`.
