...
> **현재 활성 작업: "🎓 M10 가이드 모드"** — 라운드 11 구현·수정·재리뷰까지 전부 통과됐다(코드 작업 없음). 이제 남은 건 사용자가 직접 하는 카카오 로그인 콘솔 설정(M10)·검증·배포다. Codex는 코드를 고치지 말고, 아래 "🎓 M10 가이드 모드" 섹션을 따라 **사용자를 한 단계씩 안내하는 선생님 역할**을 할 것. 라운드 1~11 체크리스트는 전부 완료됨.
