...
> **T97r 규칙은 Claude가 수정·배포함** — roster 필드 화이트리스트(create/self-update)에 `talent` 추가 + 관리자 update의 talent 음수 방지.
...
- [ ] **T94. 첫 화면 — 소셜 우선 + 공동체 등록 안내**
  - 재방문 기억(saveLastChurch)의 **자동 폼 스킵 제거** — 누구에게나 항상 소셜 첫 화면(카카오·구글)이 기본. 기억된 교회는 "기존 회원 로그인" 링크 옆에 "지난번: ○○교회" 뱃지로만. `?church=` 링크도 동일(소셜 화면 유지).
  - 첫 카드에 공동체 안내 1줄 추가: "⛪ 교회·모임(공동체)과 함께 읽으려면 **공동체 등록**이 먼저 필요해요" + [공동체 등록하기] → 기존 교회 등록(adminSignup) 진입. 상단 "교회 등록 →" 버튼 라벨도 "공동체 등록"으로 통일.
...
완료 기준: 빌드 + `npm run validate`(라운드 17에서 통합) + 지갑 이관·다중 적립 픽스처, 375px 확인. 커밋 금지(Claude 담당).
