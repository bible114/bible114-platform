...
> **현재 활성 작업: T125e 남은 운영 writer·통계 의미 확정, 그와 독립적인 T127 선행 `handleRestart` 서버 이관**. T123 읽기·퀴즈와 T125d `publicChurches` 안전 백필 기반은 2026-07-16 로컬 구현·검증까지 완료했지만 아직 배포하지 않았다. 따라서 T126 영상 경로의 기존 관찰 시계(2026-07-23 05:14 KST)는 유지하되, 읽기·퀴즈·T125를 포함한 **최종 T127 7일 시계는 최신 Edge·웹 배포 시점부터 다시 시작**한다. 그 전에는 보호 필드 직접 쓰기 차단과 legacy `videoAutoConfig.apiKey` 정리를 진행하지 않는다.
...
- 통계 결정과 독립적으로 다음 구현은 T127의 선행 조건인 `useUserBibleActions.handleRestart` 서버 action 이관이다.
