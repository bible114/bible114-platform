...
- users 보호 필드를 닫기 전에 `useUserBibleActions.checkAchievements` 등 남은 보호 필드 직접 writer를 다시 전수 감사하고 필요한 서버 action 이관을 끝낸다. `handleRestart`와 `BibleQuizCard.skipToday`는 이미 서버 transaction으로 옮겼다.
