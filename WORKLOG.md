...
## 2026-07-09 ~ 07-12 — 무소속·게스트·관리자 개편·달란트 경제·다중 소속·개인 계정 (Codex 협업 라운드 1~9)

> 상세 설계·체크리스트·리뷰 결과는 전부 **`HANDOFF_CODEX.md`** 에 있다 (Claude 설계·리뷰 / Codex 구현 왕복 문서).
> 다음 세션은 그 문서의 "📮 Codex → Claude 메모"와 최신 "✅ Claude 리뷰 결과"부터 읽을 것.

- **라운드 1**: 무소속 성도 가입(가상 교회 `unaffiliated_v1`, 표시명 「성경 읽는 사람들」) + 게스트(익명 인증+localStorage) 읽기 + 진도 이관 + 관리자 교회 이동
- **라운드 2**: 교회 관리자 화면 개편 (대시보드 탭·AdminDataTable·SlideOver·일괄 작업)
- **라운드 3**: 달란트 하루 1회 적립(10+연속≤7) + 매일 성경퀴즈 + 비밀 달란트 상점(7일 해금·opt-in) + 관리자 상점 탭 + 전원 잔액 리셋 버튼
- **라운드 4~5**: 매일 영상 자동화 마무리 + 본문 연동 퀴즈 v2(90일 문항 은행) + 점검 수정(중복 제출 방지·주간 읽기왕·완독 축하 등)
- **라운드 6**: 교회 관리자 Google 로그인/등록/계정 연결
- **라운드 7**: 교회 내 다중 소속(extraMemberships ≤3)
- **라운드 8**: 조직 간 소속 — `churches/{org}/roster/{uid}` 명부, 진도 동기화, 관리자 외부 멤버 경계
- **라운드 9**: 개인 우선 가입(accountType personal, churchId null) + 공동체 온보딩 + 기준 공동체 전환
- **Claude 직접 작업**: 로그인 첫 화면 입구 선택 개편("어떻게 읽으실래요?"), 검색 숨김 토글(테스트 교회), 창구 판매(관리자 대리 차감), A4 인쇄 3종(상품 목록/성도 가입 안내문 QR/관리자 매뉴얼), 이모지 그룹 탭, 관리자 화면 광고 숨김, 자격증명 private/auth 분리 + 교인 랭킹 규칙 수정, talentPurchases 하위 컬렉션 규칙

### 남은 일 (2026-07-12 기준)
1. **firestore.rules 배포 대기**: roster self-update 화이트리스트+joinedAt 불변 (커밋됨, `firebase deploy --only firestore:rules` 사용자 실행 필요)
2. 배포 후 실환경 검증: 개인 가입→온보딩→공동체 추가/탈퇴, 다중 소속 랭킹, Google 관리자 로그인(M8), 창구 판매
3. 퀴즈 문항(상식 113 + 본문 연동 90일치) 신학적 검수 — 사용자 몫
4. 달란트 전원 리셋 버튼 — 상점 실오픈 시 1회 실행
5. 다음 설계 후보: 메모 하위 컬렉션 분리(T35 노출 이슈), roster 4행 경합의 서버 함수 해결, 라운드 10(미정 — 사용자 결정 대기)
