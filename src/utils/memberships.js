...
export const getMembershipList = (user) => {
    ...
    const extraMemberships = Array.isArray(user.extraMemberships)
        ? user.extraMemberships.slice(0, MAX_EXTRA_MEMBERSHIPS)
        : [];
    ...
    candidates.forEach(candidate => {
        const membership = candidate === primaryMembership ? candidate : normalizeMembership(candidate);
