...
export const useTTS = (verseText) => {
    ...
    const removeVerseNumbersForTTS = (line) => {
        return line
            .replace(/\[\[VERSE:\d+\]\]/g, '')
            .replace(/(^|[\s\u00A0])\d{1,3}[.)]?\s*(?=(?![월년일시분초세살장절편권번차회명층개])[\uAC00-\uD7A3"“‘'「『])/g, '$1')
            .trim();
    };
    ...
    const handleSpeak = (text, startFromIndex = 0, overrideSpeed = null) => {
        ...
        const chunks = [];
        lines.forEach(line => {
            // 절 번호는 화면에는 보이지만 TTS에서는 읽지 않도록 제거
            const cleanLine = removeVerseNumbersForTTS(line);
            if (!cleanLine) return;
