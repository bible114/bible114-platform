...
const MarkdownRenderer = ({ content, fontSize = 16, activeChunkIndex = -1, onSegmentClick = null }) => {
    ...
    const renderLine = (line, isFirstLine, key) => {
        ...
        if (trimmed.includes('[[VERSE:')) {
            const verseBlocks = trimmed
                .split(/(?=\[\[VERSE:\d+\]\])/g)
                .filter(Boolean);
            return verseBlocks.map((block, blockIdx) => {
                const idx = segmentCounter++;
                return (
                    <p key={`${key}-verse-${blockIdx}`} id={`tts-segment-${idx}`} onClick={() => onSegmentClick && onSegmentClick(idx)} className={`text-slate-700 leading-relaxed mb-3 font-normal cursor-pointer ${activeChunkIndex === idx ? 'bg-yellow-200' : ''}`} style={{ fontSize: `${fontSize}px`, textAlign: 'justify' }}>
                        {renderInteractiveText(block, false)}
                    </p>
                );
            });
        }
